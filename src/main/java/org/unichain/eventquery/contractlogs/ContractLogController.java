...
public class ContractLogController {
  ...
  @RequestMapping(method = RequestMethod.GET, value = "/contractlogs/contract/{contractAddress}")
  ...
  public ContractLogTriggerEntity getEvent(@PathVariable(value = "uniqueId", required = false) String uniqueId) {
    QueryFactory query = new QueryFactory();
  ...
  public Long totalContractLog() {
    QueryFactory query = new QueryFactory();
    long total = mongoTemplate.count(query.getQuery(), ContractLogTriggerEntity.class);
    return total;
  }
}
