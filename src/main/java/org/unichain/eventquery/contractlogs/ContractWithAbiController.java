...
import org.unichain.eventquery.contractevents.ContractEventTriggerEntity;
...
public class ContractWithAbiController {
  ...
  @RequestMapping(method = RequestMethod.POST, value = "/contract/contractAddress/{contractAddress}")
  ...
  public JSONObject findOneByTransaction(@PathVariable String transactionId, @RequestBody Map<String, String> hmap) {
    QueryFactory query = new QueryFactory();
  ...
  private JSONObject getContractTrigger(Map<String, String> hmap, QueryFactory query) {
    List<ContractLogTriggerEntity> contractLogTriggerList = mongoTemplate.find(query.getQuery(), ContractLogTriggerEntity.class);
    List<ContractEventTriggerEntity> contractEventTriggerList = mongoTemplate.find(query.getQuery(), ContractEventTriggerEntity.class);
    Map map = new HashMap();
